#################
# rpart to separate binary happy or not happy
#################
...
# convert Happiness to just Happy or Unhappy
binhap <- ifelse(d$Happiness %in% c("Very happy", "Quite happy"), 1, 0)
d$Happiness <- factor(binhap, labels=c("Unhappy", "Happy"))
...
save(resamps, rpart2Fin, rpart2Sin, file=file.path(datapath, "FinvsSin_Binary.Rdata"))
